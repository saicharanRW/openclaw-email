...
1. **Parse extracted_text** from each document
2. **Extract relevant sections** that relate to the email query
3. **Format as concise facts** (bullet points)
...
1. **Parse the matched documents** for data that fits these columns
2. **Create data rows** by mapping facts to columns
3. **Fill each column** with the best matching value from the extracted text
4. **Use empty string `""`** if no matching data found for a column
...
1. **Look for exact matches** in the text (e.g., if column is "Vehicle No.", search for vehicle numbers)
2. **Use pattern matching** (e.g., dates, numbers, IDs)
3. **Extract from tables** if the text contains tabular data
4. **Parse structured sections** (e.g., line-by-line data)
5. **Create multiple rows** if multiple records found
6. **Limit to 50 rows** per table to avoid overwhelming the response
...
- **tables**: Primary format - list of {title, headers, rows} - one per table in AVAILABLE TABLES
...
4. ✅ **NEVER fabricate facts** - Only use actual database content
5. ✅ **ALWAYS provide source attribution** - Tell user which file(s) data came from
6. ✅ **Map to detected columns** - If columns provided, structure data accordingly
7. ✅ **Respect the format** - The pipeline creates reply in same format as received
8. ✅ **Extract keywords first** - Don't query DB without analyzing email first
9. ✅ **Use targeted queries** - Don't fetch entire database
10. ✅ **Handle empty results gracefully** - Provide helpful "not found" message
